...
- **Восстановление из backup на экране Agents теперь чинит все файлы.** Экран вызывал `restorePrimaryBackupFile` и возвращал только первый файл манифеста, а затем удалял backup — для multi-file манифеста Codex (`project` scope) остальные файлы оставались пропатченными без пути назад. Теперь используется общий `restoreBackupForAgent` (тот же путь, что и `agento restore`).
- **Ошибки в статус-строках больше не зелёные.** `Error: ...` после неудачного удаления провайдера/профиля отрисовывался тем же зелёным, что и успех. Общий компонент `StatusLine` красит ошибки в красный и скрывает сообщение через 5 секунд — залипшие «Deleted» больше не выдают повторное удаление.
- **Экран Settings врал про scope.** Описание `global` ссылалось на `~/.agento/config.json` — это конфиг самого AgentO, а не то, что меняет настройка (патчится глобальный конфиг агента: `~/.claude/settings.json` и т.п.). Описания переписаны по фактическому поведению, попутно добавлен пропавший из UI тумблер `mergeAgentConfigs`.
- **Подтверждение удаления не проглатывает ввод.** `y` работал только строчной латиницей (Caps Lock/Shift молча отменял), а «любая другая клавиша = отмена» делало случайные нажатия неотличимыми от отмены. Подтверждение: `y`/`Y`/Enter — удалить, `n`/`N`/Esc — отменить, остальной ввод игнорируется. То же в confirm «удалить профиль без моделей» в редакторе профилей.
- **Esc в форме провайдера не «залипает» на полях models/save.** Раньше с этих полей Esc отводил курсор на строку назад (нужно было два нажатия) и никак не подтверждал потерю ввода. Теперь Esc всегда отменяет форму, а при несохранённых правках спрашивает «Discard changes?».
- **Удаление последней модели из деталей профиля не игнорируется молча.** Схема запрещает профиль без моделей (`min(1)`), но сообщение об ошибке никуда не выводилось — нажатие `d` выглядело сломанным. Теперь показывается подсказка, как удалить модель через редактор.
- **«Overwrite and launch» предлагается только когда оно помогает.** Раньше вариант перезапуска через restore backup показывался при любой ошибке запуска (включая permission denied). Теперь он предлагается только при распознавании `Active backup already exists`.

### Changed

- **Единый язык интерфейса — английский.** `ProviderForm` (тест API, выбор моделей), `AgentInstall` и `AgentAction` были на русском в иначе английском TUI; все строки интерфейса приведены к английскому. Guard-тест `src/tui/english-ui.test.ts` не пускает кириллицу в UI-строки `src/tui` (комментарии могут быть на любом языке).
- **Мастер запуска показывает и позволяет менять режим/область применения.** Внизу экранов profile/agent — футер `mode: child (m) | scope: project (s)` и профиль по курсору; `m`/`s` переключают для этого запуска без похода в Settings.
- **`agent-status.json` хранит версии.** `checkInstalled()` всегда возвращал версию, но кэш терял её; теперь статус — объект `{installed, version?}` (старые boolean-файлы читаются и нормализуются), а `AgentSelect` показывает `v1.2.3` у установленных агентов.
- **Выбор моделей, полученных из API, получил поиск.** В длинных списках (у OpenRouter сотни строк) набор символов фильтрует список, Backspace правит фильтр, PageUp/PageDown перемещают на экран; Space/Enter работают как раньше.
- **Экран Agents показывает путь к конфигу** выделенной строки — строки global/project одного агента различались только им.
- **Главное меню: подсказка клавиш и быстрые цифры.** `1-5` открывают пункт сразу (с фокусом на нём), внизу — строка `↑↓ navigate | Enter open | 1-5 jump | q/Esc quit`.
- `agento profile add -m` и `agento profile list` принимают имя провайдера вместо UUID (`Acme:model:base`, регистронезависимо; UUID тоже работает). Неизвестное имя — внятная ошибка вместо падения валидации.

### Added

- Тесты ранее не покрытых частей TUI: экран Agents (multi-file restore через реальный UI), Settings, подтверждение удаления в Providers/Profiles, Esc-поведение и фильтр в ProviderForm, футер мастера запуска, `agento profile add`, кэш статусов.
