...
**Навигация:** **↑↓** перемещение, **Enter** выбор, **1-5** переход к пункту, **Esc / q** выход.

> Подсказки и пункты меню — на английском: горячие клавиши работают и при русской раскладке (ЙЦУКЕН прозрачно маппится на QWERTY).

### Обзор экранов

| Экран | Возможности | Горячие клавиши |
|-------|-------------|-----------------|
| **Запуск агента** | Выбор профиля → агента → запуск; статус установки с версией кешируется на диск; при ENOENT — перезапуск TUI с ошибкой. Футер показывает эффективные профиль/режим/область. На установленном агенте: `u` — обновить, `d` — удалить | **Enter** выбор, **Esc** назад, **u** обновить, **d** удалить, **m** режим, **s** область |
...
| **Агенты** | Проверка статуса конфигов (global/project), наличие бэкапов; выделенная строка показывает путь к конфигу | **r** восстановить (если изменён), **↑↓** навигация, **Esc** назад |
| **Настройки** | Режим запуска, область конфига и склейка конфигов агентов (mergeAgentConfigs); выделенная настройка показывает описание текущего значения | **↑↓** навигация, **Enter/Space** переключение, **Esc** сохранить и назад |

### Сценарий запуска агента

1. **Выбор профиля** — выберите один из сохранённых профилей
2. **Выбор агента** — AgentO проверяет статус установки всех агентов (спиннер). Неустановленные агенты отмечены `(not installed)`, у установленных показывается версия (`v1.2.3`), при выделении — `(u update, d delete)`. Статусы (установлен + версия) кешируются в `~/.agento/agent-status.json` — при следующем открытии уже известные агенты не перепроверяются.

   Внизу обоих шагов — футер `Profile / mode / scope`: **m** переключает child/independent, **s** — project/global для этого запуска, без похода в Настройки.
...
agento profile add [options]                  # Добавить профиль
  ...
  -m, --models <models>     Список provider:modelName[:tier] через запятую (обязательно)
                            provider — имя провайдера (без учёта регистра) или UUID
